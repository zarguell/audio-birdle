...
def filter_taxonomy(
    region_file: Path, taxonomy_file: Path, exclude_hybrids: bool = False
):
    ...
    filtered_taxonomy = [
        ...
        if entry.get("speciesCode") in region_species_codes
        and (not exclude_hybrids or entry.get("category") != "hybrid")
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Filter eBird taxonomy by region species codes."
    )
    parser.add_argument(
        "region_file",
        type=Path,
        help="Path to region JSON file (e.g. ./data/regions/us.json)",
    )
    parser.add_argument(
        "taxonomy_file", type=Path, help="Path to ebird-taxonomy.json file"
    )
    parser.add_argument(
        "--exclude-hybrids",
        action="store_true",
        help="Exclude hybrid species from output",
    )

    args = parser.parse_args()
    filter_taxonomy(
        args.region_file, args.taxonomy_file, exclude_hybrids=args.exclude_hybrids
    )
