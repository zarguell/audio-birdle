    ...
    for u in nextPageUrls[:maxUrls]:  # Limit to maxUrls
        df = pd.concat(
            [
                df,
                pd.DataFrame(
                    {"code": speciesCode, "page Url": u.get("href")}, index=[0]
                ),
            ],
            ignore_index=True,
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Scrape audio URLs from eBird taxonomy data."
    )
    parser.add_argument(
        "taxonomy_file",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Maximum number of audio URLs to get per species (default: 10)",
    )
    parser.add_argument(
        "--region", type=str, default="US", help="Region code (default: US)"
    )
    parser.add_argument(
        "--tag", type=str, default="song", help="Media tag (default: song)"
    )
    ...
    if len(audioUrls) != len(urlDF):
        print(
            f"⚠️  Warning: Length mismatch - {len(audioUrls)} audio URLs vs {len(urlDF)} page URLs"
        )
