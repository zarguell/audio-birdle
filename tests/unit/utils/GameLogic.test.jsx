...
describe("GameLogic", () => {
  ...
  describe("processGuess", () => {
    ...
    it("should not allow guesses after completion", () => {
      ...
      const beforeLength =
        state.dailyGames["us-2025-12-27-normal"].guesses.length;
  ...
  describe("generateAnswerOptions", () => {
    ...
    it("should be deterministic for same inputs", () => {
      const correctBird = sampleBirds.us[0];
      const options1 = generateAnswerOptions(
        "us",
        sampleBirds,
        "2025-12-27",
        correctBird,
      ...
      const options2 = generateAnswerOptions(
        "us",
        sampleBirds,
        "2025-12-27",
        correctBird,
  ...
  describe("getDailyBirdWithFallback", () => {
    ...
    it("should fall back to hash selection on network failure (offline mode)", async () => {
      getTodaysBirdFromDaily.mockResolvedValue({
        success: false,
        error: "network",
      });
      ...
      expect(result.bird.id).toBe(
        getDailyBird("us", sampleBirds.us, "2025-12-27").id,
      );
    });
  ...
  describe("processHardModeGuess", () => {
    ...
    it("should not allow guesses after hard mode completion", () => {
      ...
      const beforeLength =
        state.dailyGames["us-2025-12-27-hard"].guesses.length;
  ...
  describe("hasPlayedHardModeRegionDate", () => {
    it("should return false for unplayed hard mode game", () => {
      ...
      const hasPlayed = hasPlayedHardModeRegionDate(
        gameState,
        "us",
        "2025-12-27",
      );
  ...
  describe("hasCompletedNormalMode", () => {
    it("should return false for incomplete normal mode game", () => {
      const gameState = createInitialGameState();
      const key = "us-2025-12-27-normal";
      gameState.dailyGames[key] = createInitialDailyGameState(
        "us",
        "2025-12-27",
      );

      const hasCompleted = hasCompletedNormalMode(
        gameState,
        "us",
        "2025-12-27",
      );
